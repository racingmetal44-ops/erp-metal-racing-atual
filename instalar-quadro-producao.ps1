# ============================================================
# Instala o Quadro de Producao no ERP
# - Cria src/pages/ProductionBoardPage.jsx
# - Edita Layout.jsx (menu lateral)
# - Edita App.jsx (rota)
# - Edita UsersPage.jsx (lista de paginas para permissoes)
# Faz backup (.bak) de cada arquivo existente antes de editar.
# ============================================================
...
# --- checagem inicial -------------------------------------------------
$raiz = "."
$layoutPath = ".\src\components\layout\Layout.jsx"
$appPath = ".\src\App.jsx"
$usersPath = ".\src\pages\UsersPage.jsx"
$novaPaginaPath = ".\src\pages\ProductionBoardPage.jsx"

foreach ($p in @($layoutPath, $appPath, $usersPath)) {
    if (-not (Test-Path $p)) {
        Write-Host "Arquivo nao encontrado: $p" -ForegroundColor Red
        Write-Host "Rode este script na pasta raiz do projeto (onde tem a pasta 'src')." -ForegroundColor Yellow
        exit 1
    }
}

# --- 1) Cria a pagina nova ---------------------------------------------
$novaPagina = @'
...
[System.IO.File]::WriteAllText((Join-Path (Get-Location) $novaPaginaPath.TrimStart('.','\')), $novaPagina, $utf8NoBom)
Write-Host "Criado: $novaPaginaPath" -ForegroundColor Green

# --- 2) Edita Layout.jsx -------------------------------------------------
Backup-File $layoutPath
$layout = Get-Content $layoutPath -Raw -Encoding UTF8

$importAntigo = "import { LayoutDashboard, Package, ScanBarcode, Factory, ShoppingCart, FileText, Landmark, TrendingUp, Bell, Users, Tag, Truck, Tv, Settings, Bot, Table2, Map, Building2, RotateCcw, Lightbulb, ShieldCheck, Upload, LogOut } from 'lucide-react';"
$importNovo  = "import { LayoutDashboard, Package, ScanBarcode, Factory, ShoppingCart, FileText, Landmark, TrendingUp, Bell, Users, Tag, Truck, Tv, Settings, Bot, Table2, Map, Building2, RotateCcw, Lightbulb, ShieldCheck, Upload, LogOut, ClipboardList } from 'lucide-react';"

if ($layout.Contains($importAntigo)) {
    $layout = $layout.Replace($importAntigo, $importNovo)
} else {
    Write-Host "AVISO: linha de import de icones nao encontrada exatamente igual em Layout.jsx - adicione ClipboardList manualmente." -ForegroundColor Yellow
}

$navAntigo = "  { to: '/pedidos', label: 'Pedidos', icon: ShoppingCart },"
$navNovo   = "  { to: '/pedidos', label: 'Pedidos', icon: ShoppingCart },`n  { to: '/quadro-producao', label: 'Quadro de Producao', icon: ClipboardList },"

if ($layout.Contains($navAntigo)) {
    $layout = $layout.Replace($navAntigo, $navNovo)
} else {
    Write-Host "AVISO: linha de menu 'Pedidos' nao encontrada exatamente igual em Layout.jsx - adicione o item de menu manualmente." -ForegroundColor Yellow
}

[System.IO.File]::WriteAllText((Join-Path (Get-Location) $layoutPath.TrimStart('.','\')), $layout, $utf8NoBom)
Write-Host "Editado: $layoutPath" -ForegroundColor Green

# --- 3) Edita App.jsx -----------------------------------------------------
Backup-File $appPath
$app = Get-Content $appPath -Raw -Encoding UTF8

$importDashboard = "import DashboardPage from './pages/DashboardPage';"
$importComNovo = "import DashboardPage from './pages/DashboardPage';`nimport ProductionBoardPage from './pages/ProductionBoardPage';"

if ($app.Contains($importDashboard) -and -not $app.Contains("ProductionBoardPage")) {
    $app = $app.Replace($importDashboard, $importComNovo)
} else {
    Write-Host "AVISO: import do DashboardPage nao encontrado (ou ProductionBoardPage ja existe) em App.jsx - confira manualmente." -ForegroundColor Yellow
}

$rotaDashboard = '<Route path="/dashboard" element={<DashboardPage />} />'
$rotaComNova = "<Route path=`"/dashboard`" element={<DashboardPage />} />`n        <Route path=`"/quadro-producao`" element={<ProductionBoardPage />} />"

if ($app.Contains($rotaDashboard) -and -not $app.Contains("/quadro-producao")) {
    $app = $app.Replace($rotaDashboard, $rotaComNova)
} else {
    Write-Host "AVISO: rota do dashboard nao encontrada (ou rota nova ja existe) em App.jsx - confira manualmente." -ForegroundColor Yellow
}

[System.IO.File]::WriteAllText((Join-Path (Get-Location) $appPath.TrimStart('.','\')), $app, $utf8NoBom)
Write-Host "Editado: $appPath" -ForegroundColor Green

# --- 4) Edita UsersPage.jsx (lista de paginas para permissoes) -----------
Backup-File $usersPath
$users = Get-Content $usersPath -Raw -Encoding UTF8

$pageAntiga = "  { path: '/pedidos', label: 'Pedidos' },"
$pageNova   = "  { path: '/pedidos', label: 'Pedidos' },`n  { path: '/quadro-producao', label: 'Quadro de Producao' },"

if ($users.Contains($pageAntiga) -and -not $users.Contains("/quadro-producao")) {
    $users = $users.Replace($pageAntiga, $pageNova)
} else {
    Write-Host "AVISO: linha 'Pedidos' nao encontrada (ou ja existe) em UsersPage.jsx - confira manualmente." -ForegroundColor Yellow
}

[System.IO.File]::WriteAllText((Join-Path (Get-Location) $usersPath.TrimStart('.','\')), $users, $utf8NoBom)
Write-Host "Editado: $usersPath" -ForegroundColor Green

Write-Host ""
Write-Host "PRONTO! Quadro de Producao instalado." -ForegroundColor Cyan
Write-Host "Nao esqueca de rodar o SQL supabase-production-board.sql no Supabase se ainda nao rodou." -ForegroundColor Yellow
Write-Host "Depois, suba pro GitHub:" -ForegroundColor Yellow
Write-Host "  git add ." -ForegroundColor Yellow
Write-Host "  git commit -m 'adiciona quadro de producao'" -ForegroundColor Yellow
