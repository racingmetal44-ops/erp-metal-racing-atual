...
class NfeSoapService {
    ...
    static async carregarCadeiaCA() {
        ...
        if (!await fs.pathExists(intermediariaPath)) {
            throw new Error(
                `Certificado CA intermediéf¡rio néf£o encontrado: ${intermediariaPath}`
            );
        }

        if (!await fs.pathExists(raizPath)) {
            throw new Error(
                `Certificado CA raiz néf£o encontrado: ${raizPath}`
            );
        }
        ...
        console.log(
            '[SEFAZ] CA intermediéf¡ria carregada:',
    ...
    static obterWebService(empresa, ambiente, servico) {
        ...
        if (!config) {
            throw new Error(
                `Endpoint SEFAZ néf£o encontrado para cUF=${cUF}, tpAmb=${tpAmb}, serviéf§o=${servico}.`
            );
        }
    ...
    static montarSoapAutorizacao(
        ...
        // Remove a declaraéf§éf£o XML para que o XML da NF-e
        ...
        // Lote éfºnico para processamento séf­ncrono.
        // O identificador deve ter no méf¡ximo 15 déf­gitos.
        const idLote =
            String(Date.now())
                .replace(/\D/g, '')
                .slice(-15)
...
<soap12:Envelope
    ...
    static async enviar(
        ...
        try {
            ...
            throw new Error(
                `Erro de comunicaéf§éf£o com a SEFAZ: ${error.message}`
            );
        }
    }
    ...
    static async consultarRecibo(
        ...
        if (!nRec) {
            throw new Error(
                'Nºfºmero do recibo (nRec) néf£o informado.'
            );
        }
...
<soap12:Envelope
    ...
    static async testarStatus(
        ...
        try {
            ...
            return {
                ...
                cStat: null,
                xMotivo:
                    `Erro de comunicaéf§éf£o: ${error.message}`,
                tempoMs:
                    Date.now() - inicio,
                httpStatus:
    ...
    static parseSefazResponse(xmlResponse) {
        ...
        // =====================================================
        // EXTRATOR GENºfé?éRICO DE TAG XML
        ...
        if (statusMatch) {
            ...
            return {
                ...
                xMotivo:
                    xMotivo ||
                    'Retorno do serviéf§o de status da SEFAZ.',
