    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6">
        ...
        <p className="mt-2 text-sm text-slate-400">
          Cadastro, edição e controle de estoque com integração real ao Supabase.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-4">
          <div className="flex items-center gap-2 text-emerald-400">
            <Package size={18} />
            <span className="text-sm">Total em Estoque</span>
          </div>
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6">
        ...
        <form onSubmit={handleSubmit} className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          <input
            className="rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-white placeholder:text-slate-500 focus:border-orange-500 focus:outline-none"
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 rounded-xl border border-slate-700 bg-slate-950/50">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setForm({ ...form, max_stock: Number(e.target.value) || 0 })}
                  min="0"
                />
              </div>
            </div>
              ...
              <div className="mt-3 grid gap-3 grid-cols-2 sm:grid-cols-3 md:grid-cols-4">
                {gallery.map((image) => (
                  <div key={image.id} className="rounded-xl border border-slate-800 bg-slate-950/70 p-2">
                    <button
                      type="button"
                      onClick={() => openImageModal(gallery, 0)}
                      className="block w-full"
                    >
                      <img
                        src={image.file_url}
                        alt={image.file_name || 'Imagem'}
                        className="h-24 w-full rounded-lg object-cover"
                      />
                    </button>
                    <p className="mt-2 text-xs text-slate-400 truncate">
                      {image.file_name || 'Imagem salva'}
                    </p>
                  </div>
                ))}
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6">
        <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex gap-2 w-full md:w-auto">
            <button
              onClick={abrirModalBipe}
              className="flex items-center gap-2 rounded-xl bg-emerald-500/20 px-4 py-2 text-sm font-semibold text-emerald-400 hover:bg-emerald-500/30 transition"
            >
              <QrCode size={16} />
              Bipar Produto
            </button>
            <div className="relative flex-1 md:w-80">
              ...
              <input
                ...
                className="w-full rounded-xl border border-slate-700 bg-slate-950 px-10 py-2 text-white placeholder:text-slate-500 focus:border-orange-500 focus:outline-none"
              />
            </div>
          </div>
        </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {filtered.map((product) => {
              ...
              return (
                <div
                  key={product.id}
                  className={`rounded-3xl border p-4 shadow-lg shadow-slate-950/10 transition hover:shadow-slate-950/30 ${
                    stockStatus.label === 'Baixo'
                      ? 'border-rose-500/30 bg-rose-500/5'
                      : stockStatus.label === 'Alto'
                      ...
                      >
                        ...
                        <div className="absolute inset-0 bg-black/0 hover:bg-black/20 transition-all duration-200 flex items-center justify-center">
                          <span className="bg-black/60 text-white px-3 py-1 rounded-full text-xs opacity-0 group-hover:opacity-100 transition-opacity">
                            {imageCount > 1 ? `Ver ${imageCount} fotos` : 'Ver foto'}
                          </span>
                        </div>
                      </button>
                    ) : (
                  ...
                  <div className="mt-4 space-y-2">
                      ...
                      <div className="rounded-xl bg-slate-900/50 p-2 text-center">
                        ...
                        <p className={`font-bold ${
                          (product.current_stock ?? 0) < (product.min_stock ?? 0)
                            ? 'text-rose-400'
                            : (product.current_stock ?? 0) > (product.max_stock ?? 99999)
                            ? 'text-amber-400'
                            : 'text-emerald-400'
                        }`}>
                    ...
                    {product.category && (
                      <p className="text-sm text-slate-400">
                        <span className="text-slate-500">Categoria:</span> {product.category}
                      </p>
                    )}
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/80 p-4">
          <div className="relative w-full max-w-lg rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-2xl">
            ...
            <input
              ref={bipeInputRef}
              type="text"
              className="w-full rounded-xl border-2 border-orange-500/50 bg-slate-950 px-4 py-4 text-center text-2xl font-mono text-white placeholder:text-slate-600 focus:border-orange-500 focus:outline-none"
              placeholder="Digite ou leia o código..."
              value={bipeCode}
              onChange={(e) => setBipeCode(e.target.value)}
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
                  buscarProdutoParaBipe(bipeCode);
                }
              }}
              autoFocus
            />
            ...
            {bipeLoading && (
              <div className="mt-4 text-center text-slate-400">
                <span className="animate-pulse">Buscando produto...</span>
              </div>
            )}
              ...
              <div className="mt-4 rounded-xl border border-slate-700 bg-slate-950/70 p-4">
                <div className="flex items-center gap-4">
                  {bipeProduct.images && bipeProduct.images.length > 0 ? (
                    <img
                      src={bipeProduct.images[0].file_url}
                      alt={bipeProduct.name}
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-1 flex items-center gap-2">
                      ...
                      <span className={`text-xl font-bold ${
                        (bipeProduct.current_stock ?? 0) < (bipeProduct.min_stock ?? 0)
                          ? 'text-rose-400'
                          : 'text-emerald-400'
                      }`}>
                ...
                <div className="mt-4 flex gap-3">
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-emerald-500 px-4 py-3 font-semibold text-white hover:bg-emerald-600 transition"
                  >
                    <Plus size={18} />
                    Adicionar (+1)
                  </button>
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-rose-500 px-4 py-3 font-semibold text-white hover:bg-rose-600 transition disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <Minus size={18} />
                    Remover (-1)
                  </button>
            ...
            {!bipeProduct && !bipeLoading && bipeCode && (
      ...
      {modalImage && modalImages.length > 0 && (
        ...
        >
          ...
          >
            <button
              onClick={closeImageModal}
              className="fixed top-4 right-4 text-white hover:text-orange-400 transition p-2 z-20 bg-black/50 rounded-full hover:bg-black/70"
            >
              <X size={32} />
            </button>

            <div className="fixed top-4 left-4 text-white/80 text-sm z-20 font-medium bg-black/50 px-3 py-1 rounded-full">
            ...
            <div className="relative w-full max-w-6xl mx-4 flex items-center justify-center">
              ...
              <button
                ...
                className="fixed left-4 top-1/2 -translate-y-1/2 bg-black/60 hover:bg-orange-500 text-white p-4 rounded-full transition-all duration-200 hover:scale-110 shadow-lg z-20"
              >
                <ChevronLeft size={40} />
              </button>
              ...
              <button
                ...
                className="fixed right-4 top-1/2 -translate-y-1/2 bg-black/60 hover:bg-orange-500 text-white p-4 rounded-full transition-all duration-200 hover:scale-110 shadow-lg z-20"
              >
