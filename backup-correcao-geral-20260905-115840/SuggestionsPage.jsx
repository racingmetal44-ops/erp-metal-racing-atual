...
export default function SuggestionsPage() {
  ...
  async function handleSubmit(e) {
    ...
    if (editingId) {
      ...
      if (!error) {
        setMessage('Sugestáo atualizada com sucesso.');
        setEditingId(null);
        setForm(emptyForm);
    ...
    if (!error) {
      setMessage('Sugestáo criada com sucesso.');
      setForm(emptyForm);
      await loadSuggestions();
    } else {
      setMessage(error.message);
  ...
  async function handleDelete(id) {
    ...
    if (!error) {
      setMessage('Sugestáo removida com sucesso.');
      await loadSuggestions();
    } else {
      setMessage(error.message);
    }
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6">
        <p className="text-sm text-orange-400">Sugestáes</p>
        <h1 className="mt-2 text-3xl font-semibold">Sugestáes</h1>
        <p className="mt-2 text-sm text-slate-400">Solicitações e ideias para melhoria do processo.</p>
      </div>

      <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6">
        <h2 className="text-lg font-semibold">{editingId ? 'Editar sugestáo' : 'Nova sugestáo'}</h2>
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6">
        <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <h2 className="text-lg font-semibold">Sugestáes cadastradas</h2>
          ...
          <div className="space-y-3">
            {filtered.map((suggestion) => (
              <div key={suggestion.id} className="rounded-2xl border border-slate-800 bg-slate-950/60 p-4">
                <h2 className="font-semibold">{suggestion.title || 'Sugestáo'}</h2>
