...
setProdutosERP(data || []);
    ...
    async function consultarSefaz() {
        ...
        if (bloqueadoAte > agora) {
            ...
            setErro(
                '?x? A consulta é SEFAZ está temporariamente bloqueada por consumo indevido (cStat 656). Aguarde 1 hora antes de consultar novamente.'
            );
        ...
        try {
            ...
            if (
                ...
                setErro(
                    `?x? SEFAZ bloqueou temporariamente a consulta. cStat 656 ? Consumo Indevido. Aguarde 1 hora antes de consultar novamente.`
                );
            ...
            if (data.success) {
                ...
                setErro(
                    `?R SEFAZ: ${data.xMotivo || data.error || 'Consulta sem sucesso fiscal.'}`
                );
            }
    ...
    async function manifestar(entrada, tipoEvento) {
        ...
        const justificativa =
            tipoEvento === '210240'
                ? window.prompt(
                    'Operação não Realizada exige justificativa (ménimo 15 caracteres):'
                )
                : null;

        if (tipoEvento === '210240' && (!justificativa || justificativa.trim().length < 15)) {
            setErro('Justificativa obrigatéria (ménimo 15 caracteres) para Operação não Realizada.');
            return;
        }
        ...
        try {
            ...
            setMensagem(`?S& ${data.message} (cStat ${data.cStat})`);
    ...
    async function importarXML() {
        ...
        try {
            ...
            setMensagem(
                '?S& XML importado para conferência.'
            );
    ...
    async function cadastrarManual() {
        ...
        try {
            ...
            setMensagem(
                '?S& Entrada manual criada para conferência.'
            );
    ...
    // transaééo e rollback. O frontend apenas
    // envia os vénculos.
    // =========================================
    // =========================================
    // CONFIRMAR ENTRADA
    ...
    async function confirmarEntrada(entrada) {
        ...
        const confirmado = window.confirm(
            '?a?? ESTA OPERA?!éO IRé ALTERAR O ESTOQUE.\n\n' +
            `NF-e ${entrada.numero || '-'} - ` +
            `${entrada.fornecedor?.razaoSocial || 'fornecedor'}\n` +
            `${produtos.length} item(ns) receberéo entrada de estoque.\n\n` +
            'Produtos não cadastrados seréo criados automaticamente.\n\n' +
            'Confirma a entrada?'
        );
        ...
        try {
            ...
            if (sefaz.success) {
                ...
                mensagemSefaz =
                    '\n?S& CONFIRMA?!éO DA OPERA?!éO ACEITA PELA SEFAZ' +
                    `\ncStat: ${sefaz.cStat || '-'}` +
                ...
                mensagemSefaz =
                    '\n?a?? ENTRADA REALIZADA, MAS A MANIFESTA?!éO NºO FOI ACEITA PELA SEFAZ' +
                    `\ncStat: ${sefaz.cStat || '-'}` +
                ...
                mensagemSefaz =
                    '\n?a?? ENTRADA REALIZADA NO ERP' +
                    '\nManifestação não enviada é SEFAZ' +
                    `\nMotivo: ${sefaz.xMotivo || '-'}`;
            }

            setMensagem(
                '?S& Entrada realizada com sucesso.' +
    ...
    return (
        <div className="space-y-6">
            ...
            {aba === 'sefaz' && (
                ...
                <div className="p-4 rounded-xl border border-slate-700 bg-slate-800/50 space-y-4">
                    ...
                    <h2 className="text-lg font-semibold text-white">
                        ?xR? Consultar SEFAZ (Distribuição DF-e)
                    </h2>
                    ...
                    {tempoBloqueioSefaz > 0 && (
                        <div className="p-4 rounded-lg bg-red-900/30 border border-red-700 text-red-300">
                            <div className="font-bold">
                                ?x? CONSULTA é SEFAZ TEMPORARIAMENTE BLOQUEADA
                            </div>
                            ...
                            <div className="text-lg font-bold mt-2">
                                ?? {Math.floor(tempoBloqueioSefaz / 60000)}:
                                {String(
                                    Math.floor(
            ...
            <div className="p-4 rounded-xl border border-slate-700 bg-slate-800/50">
                ...
                <input
                    placeholder="?x} Buscar por chave, CNPJ, fornecedor ou número..."
                    value={busca}
                                    ...
                                    {entrada.status !== 'CONFIRMADA' && (
                                        ...
                                        <div className="mt-4 space-y-3">
                                            ...
                                            <div className="p-3 rounded-lg bg-yellow-900/30 border border-yellow-700 text-yellow-300 text-sm">
                                                ?a?? Esta operação iré alterar o estoque.
                                            </div>
