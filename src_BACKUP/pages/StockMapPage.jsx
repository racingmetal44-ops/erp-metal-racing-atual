  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6">
        ...
        <p className="mt-2 text-sm text-slate-400">Visualização do layout fésico e localizaééo de itens.</p>
      </div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {['Zona A', 'Zona B', 'Zona C'].map((zone) => (
            <div key={zone} className="rounded-xl border border-slate-800 bg-slate-950/60 p-4">
              ...
              <p className="mt-2 text-sm text-slate-400">Área de armazenamento e movimentaééo.</p>
            </div>
          ))}
        </div>
      </div>
