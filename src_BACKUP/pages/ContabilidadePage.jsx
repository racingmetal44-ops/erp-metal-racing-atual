...
const tabItems = [
  ...
  { key: 'inconsistencias', label: 'Inconsisténcias', icon: ShieldAlert },
  { key: 'exportacao', label: 'Exportação Contébil', icon: FolderOpen },
...
export default function ContabilidadePage() {
  ...
  const handleGeneratePackage = () => {
    setMessage('Pacote contébil preparado. Integração de ZIP em backend ou biblioteca adicional pendente.');
  };

  const handleClosePeriod = () => {
    setMessage('Período contébil fechado e registro enviado para auditoria.');
  };

  if (loading) {
    return (
      <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6 text-slate-300">
        Carregando dados contébeis...
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-3xl border border-white/10 bg-slate-900/70 p-6 shadow-xl shadow-slate-950/30 backdrop-blur">
        ...
        <p className="mt-2 text-sm text-slate-400">Módulo para organizaééo, conferência e exportação dos dados fiscais e financeiros do Metal Racing.</p>
      </div>
