...
export default function ReturnsPage() {
  ...
  async function loadReturns() {
    ...
    try {
      ...
      console.error('[DEVOLU��ES] Erro ao carregar:', error);
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      console.error('[DEVOLU��ES] Erro ao salvar:', error);
  ...
  async function handleDelete(id) {
    try {
      ...
      console.error('[DEVOLU��ES] Erro ao excluir:', error);
              ...
              filtered.map((item) => (
                <div key={item.id} className="rounded-2xl border border-slate-800 bg-slate-950/60 p-5 hover:border-slate-700 transition">
                  <div className="flex justify-between items-start">
                    <h2 className="text-lg font-semibold">#{item.order_number || item.numero_devolucao || item.pedido_id || "N/D"}</h2>
                    <span className={'text-xs px-2 py-1 rounded-full ' + (
                      item.status === 'aprovada' ? 'bg-green-500/20 text-green-400' :
                      item.status === 'rejeitada' ? 'bg-red-500/20 text-red-400' :
                      'bg-yellow-500/20 text-yellow-400'
                    )}>
                      {item.status === "aprovada" || item.status === "autorizada" ? "Aprovada" : item.status === "rejeitada" ? "Rejeitada" : "Pendente"}
                    </span>
                  </div>
                  <p className="mt-2 text-sm text-slate-400">Data: {item.return_date ? new Date(`${item.return_date}T00:00:00`).toLocaleDateString("pt-BR") : "N/D"}</p>
                  ...
                  {(item.original_nfe_number || item.nfe_original) && <p className="mt-1 text-sm text-slate-400">NF-e: {item.original_nfe_number || item.nfe_original}</p>}
                  <div className="mt-2 flex gap-3 text-sm">
                    <span className="text-slate-400">Valor: R$ {Number(item.product_value || 0).toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                    {Number(item.cost || 0) > 0 && <span className="text-slate-400">Custo: R$ {Number(item.cost).toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>}
                  </div>
