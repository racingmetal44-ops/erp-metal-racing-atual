...
import {
  Trophy, Users, TrendingUp, Search, Calendar, User,
  Crown, Zap, Target, BarChart3
} from 'lucide-react';
...
export default function RankingPage() {
  ...
  async function loadRanking() {
    setLoading(true);
    try {
      const res = await fetch(`/api/pcp/ranking-frontend?periodo=${periodo}`);
      const data = await res.json();

      if (!data.sucesso) throw new Error(data.erro || 'Erro ao carregar');

      // Adapta o formato: o backend devolve { nome, total, ordens }
      // A tela espera { nome, total, entradas, saidas }
      const rankingArray = (data.ranking || []).map((r) => ({
        nome: r.nome,
        total: r.total,
        entradas: r.total,   // consideramos todas como "entradas" (peças produzidas)
        saidas: 0,
        ordens: r.ordens || 0
      }));
      ...
      setStats({
        total_usuarios: data.stats?.total_usuarios || 0,
        total_bipagens: data.stats?.total_bipagens || 0
      });
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6">
        ...
        <p className="mt-2 text-sm text-slate-400">Os colaboradores mais produtivos</p>
      </div>
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6">
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-slate-400">Nenhuma produção registrada ainda.</p>
          </div>
        ) : (
          <div className="space-y-6">
            {/* PÓDIO (Top 3) */}
                ...
                {filteredRanking.slice(0, 3).map((item, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-sm text-slate-400">peças</p>
                      ...
                      <div className="flex justify-center gap-4 mt-3 text-xs text-slate-400">
                        <span>Peças: {item.total}</span>
                        <span>Ordens: {item.ordens}</span>
                      </div>
                    </div>
                  );
                })}
