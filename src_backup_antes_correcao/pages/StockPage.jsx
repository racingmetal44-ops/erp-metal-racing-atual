                ...
                <div key={product.id} className={`rounded-3xl border p-4 shadow-lg shadow-slate-950/10 transition hover:shadow-slate-950/30 ${stockStatus.label === 'Baixo' ? 'border-rose-500/30 bg-rose-500/5' : stockStatus.label === 'Alto' ? 'border-amber-500/30 bg-amber-500/5' : 'border-slate-800 bg-slate-950/80'}`}>
                  ...
                  <div className="mt-4 space-y-2">
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      ...
                      <button onClick={() => handleDelete(product.id)} className="rounded-2xl border border-rose-500/30 bg-slate-900 px-3 py-2 text-xs font-semiboldtext-rose-300 transition hover:bg-rose-500/10">??? Excluir</button>
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/80 p-4">
          <div className={`relative w-full max-w-lg rounded-2xl border-2 p-6 shadow-2xl transition-all duration-300 ${getBipeBgColor()}`}>
            ...
            <div className="text-center mb-6">
              ...
              <p className="text-xs text-emerald-400 mt-1">?? Leitura automética - não precisa de ENTER</p>
            </div>
            ...
            <p className="mt-4 text-center text-xs text-slate-500">?? A busca é feita automaticamente apés digitar o código</p>
          </div>
        </div>
      )}
